  ...
  },
  {
    ...
    fields: [
      ...
      },
      {
        ...
        criteria:
          "COMPOUND FACT, PER SERVICE — assess gas, electrical and water independently, but ONLY for services that the appliance list actually requires (a gas hob makes gas relevant; an all-electric kitchen does not). Every relevant service addressed = present. At least one relevant service addressed but another relevant one silent — e.g. electrical connections covered but nothing said about gas where a gas hob is listed — = AMBIGUOUS, not present, and not absent. No relevant service addressed = absent. Explicit exclusion of a service ('gas connection by a Gas Safe engineer arranged by the customer') counts as that service being addressed. CROSS-SECTION EVIDENCE IS MANDATORY (this field's most common misread): the three services are almost never described in one place. Before deciding, scan the WHOLE document — PLUMBING, ELECTRICAL, WATER/WASTE, HEADINGS OF ANY NAME — not just the appliances section. Evidence found under any heading counts exactly as if it sat next to the appliance list. Worked example: an appliances section saying 'all electrical connections for the appliances carried out by our electrician' PLUS a plumbing section saying 'new appliance valve and standpipe provided for the dishwasher' = electrical addressed AND water addressed; with an induction hob (no gas relevant) that is PRESENT, not ambiguous. Do NOT mark ambiguous because a relevant service was addressed in a different section — only genuine silence across the entire document makes a relevant service unaddressed. COLLECTIVE-NOUN / QUALITY-CLAIM RULE: 'everything connected up', 'all services sorted', 'we'll make it all work' name no service — ABSENT, never ambiguous.",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    fields: [
      ...
      },
      {
        ...
        criteria:
          "present if under-cabinet, plinth or cabinet lighting is specified (type, quantity, or explicitly included/excluded/optional with a price). COLLECTIVE-NOUN / QUALITY-CLAIM RULE (overrides everything else in this field, and matches every other category on this platform): lighting referred to with a quality or generic claim and no type, quantity, position detail, inclusion/exclusion or price — 'nice lighting under the units', 'lovely lighting', 'mood lighting sorted', 'lighting included' — is ABSENT, NEVER ambiguous, however confident the wording, because it carries no specification. AMBIGUOUS is reserved for wording that raises the specification and leaves it open — 'under-unit lighting type to be confirmed', 'LED or halogen, we'll decide on site'. NEAR-MISS = ABSENT: ceiling or general room lighting wording does NOT evidence under-cabinet lighting (NO-REUSE — different subject) — absent. Silence is absent.",
      },
      {
      ...
      },
      {
        ...
        criteria:
          "present if the document states circuit provision for appliances — a new cooker circuit, a dedicated radial for the oven, a 32A hob supply, or an explicit statement that existing circuits are being reused. AMBIGUOUS (this exact wording, decide it the same way every time): circuits are named as being provided but with no appliance and no rating — 'new circuits as required', 'new circuits as needed', 'additional circuits where necessary'. An 'as required'/'as needed'/'where necessary' qualifier does NOT make it absent: the circuit provision itself has been stated, only its extent is open, so this is AMBIGUOUS, never absent. NEAR-MISS = ABSENT: 'oven wired in', 'appliances connected' name no circuit — absent, not ambiguous (that evidence belongs to appliance_connection_gas_electric_water; NO-REUSE, different subject). Silence is absent.",
      },
    ],
  },
  {
    ...
    fields: [
      ...
      },
      {
        ...
        criteria:
          "present if the splashback is specified by material/product or area ('6mm toughened glass splashback behind the hob', 'quartz upstand to match the worktop'), or explicitly excluded. COLLECTIVE-NOUN / QUALITY-CLAIM RULE (overrides everything else in this field, and matches every other category on this platform): a splashback referred to with a quality or generic claim and no material, product or area — 'a smart splashback', 'a nice splashback', 'quality splashback fitted', 'splashback included' — is ABSENT, NEVER ambiguous, however confident the wording. AMBIGUOUS is reserved for wording that raises the specification and leaves it open — 'splashback material to be confirmed', 'glass or tiled splashback, your choice'. NEAR-MISS = ABSENT: no mention of the splashback, or only general tiling wording (NO-REUSE — tiling_scope is a different subject unless the sentence names the splashback) — absent. Silence is absent.",
      },
      {
        key: "tiling_scope",
