  ...
  },
  {
    ...
    fields: [
      ...
      },
      {
        ...
        criteria:
          "present if the document states the units meet the Building Regulations thermal standard for replacement glazing — e.g. 'meets Part L1B for replacement windows', 'compliant with the current Part L U-value limit of 1.4 W/m²K', 'Building Regulations compliant replacement glazing under Part L'. NO-REUSE (narrow): the FENSA/CERTASS registration statement is scored in its own field and does NOT by itself evidence Part L compliance here; a bare U-value figure with no compliance statement does NOT either. DECISIVE RULE: a compliance claim that makes NO reference to Part L, to the thermal standard, or to a U-value limit is ABSENT — never ambiguous. This covers 'meets Building Regulations', 'all work to current regulations', 'fully compliant installation', 'we're fully qualified', 'built to regs'. AMBIGUOUS is reserved solely for a statement that DOES invoke the thermal/Part L standard but leaves it incomplete (e.g. 'meets the Part L requirement' with no standard or figure named). Silence is absent.",
