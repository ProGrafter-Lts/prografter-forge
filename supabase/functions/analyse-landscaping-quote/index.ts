// Landscaping / Driveway Quote Checker — a self-contained, homeowner-friendly analyser.
//
// This function is INTENTIONALLY independent of every other module. It NEVER
// uses those modules' categories. It scores a landscaping / driveway / patio
// quote against landscaping-specific categories ONLY and refers to the
// tradesperson as the "landscaper".
...
// ---- Landscaping / Driveway scoring categories (this module ONLY) -----------
const CATEGORIES: { key: string; name: string }[] = [
  { key: "quote_basics", name: "Quote Basics" },
  { key: "scope_area_measurements", name: "Scope / Area / Measurements" },
  { key: "excavation_ground_prep", name: "Excavation / Ground Preparation" },
  { key: "sub_base_drainage_falls", name: "Sub-Base / Drainage / Falls" },
  { key: "materials_finish", name: "Materials / Finish Specification" },
  { key: "edging_steps_retaining", name: "Edging / Steps / Retaining Details" },
  { key: "waste_access_plant", name: "Waste Removal / Access / Plant" },
...
function buildPrompt(intake: Record<string, unknown>, supportingNames: string[]): string {
  const ctx = intake?.landscaping_context ?? intake ?? {};
  return `You are ProGrafter's LANDSCAPING / DRIVEWAY QUOTE CHECKER — a homeowner-friendly assistant.

Your job: check whether a quote for LANDSCAPING, DRIVEWAY, PATIO, FENCING, TURFING, DRAINAGE or mixed external works clearly explains excavation, ground preparation, sub-base, drainage, materials, edging/steps/retaining details, waste removal, access, exclusions, price and payment.

You are NOT the extension, boiler, electrical, bathroom, roofing, kitchen or windows/doors checker. NEVER assess or score foundations for habitable buildings, boilers, rewires, bathroom sanitaryware, roof coverings, kitchen units, windows or unrelated items. Ignore any such items entirely — this is a landscaping / driveway quote. Refer to the tradesperson as the "landscaper" (or "driveway installer" where clearly a driveway), never the "builder" or "roofer".

===== FIRST: CONFIRM THIS IS A LANDSCAPING / DRIVEWAY QUOTE =====
Look at the MAIN document. If it is clearly NOT a landscaping / driveway / patio / fencing / turfing / drainage quote (e.g. it is an extension, boiler, electrical rewire, bathroom, kitchen, roofing, windows or unrelated document), set "is_landscaping_quote": false and STOP scoring — return the minimal shape with is_landscaping_quote:false and a short note. Otherwise set "is_landscaping_quote": true and complete the full analysis.
...
===== WHAT TO CHECK (landscaping / driveway only) =====
Quote Basics: landscaper/company name, quote date, quote validity, property address, VAT status, total price, whether price is fixed or estimated.

Scope / Area / Measurements: type of work (patio / driveway / fencing / turfing / landscaping / drainage / mixed); area in m²; layout described; number of features (e.g. steps, walls); confirmation of a site visit or measured survey.

Excavation / Ground Preparation: depth of dig; spoil removal; ground conditions or unknowns flagged; existing surfaces to be removed; membrane/geotextile if used.

Sub-Base / Drainage / Falls: sub-base material (MOT Type 1, Type 3 etc.); sub-base depth; compaction; sand/cement/mortar bed; falls / gradient for water run-off; drainage (channel drains, ACO drains, soakaway, connection to existing drainage); permeable requirements for driveways (SUDS); manhole covers adjusted to level.

Materials / Finish Specification: paving / block / tarmac / resin / gravel / porcelain / natural stone type, brand, thickness, colour, laying pattern; jointing (kiln-dried sand, mortar, resin jointing); sealing/finishing if included.

Edging / Steps / Retaining Details: edging type; steps; retaining walls (materials, height, foundation); planting beds; fencing detail if included.

Waste Removal / Access / Plant: waste/spoil removal included; skip or grab lorry; access route (side gate, through house); plant hire (mini digger, dumper, breaker); protection of existing surfaces/lawn.

Exclusions / Extras / Risk Items: unknown ground conditions; buried services; existing drainage issues; utility diversions; make-good to lawn/planting; extras for lifting manholes; asbestos or contaminated ground.

Price / VAT / Payment Terms: total price, VAT inclusive/exclusive, deposit, staged payments, balance trigger, variation process for changes on site.

Timescale / Guarantees / Handover: start date, install duration, weather allowance, workmanship guarantee, material/manufacturer warranty (e.g. block paving / resin), aftercare guidance.

===== COMMON RISKS TO FLAG =====
- No dig depth stated
- No sub-base depth stated
- Drainage / falls not mentioned
- Waste removal excluded or unclear
- Poor / restricted access not addressed
- Manholes not adjusted to finished level
- Edging / retaining not included
- Material type/brand/thickness vague
- Sealing / jointing not covered
- Unknown ground conditions not flagged as a risk

===== SCORE BEHAVIOUR & CALIBRATION =====
Do NOT punish a landscaping / driveway quote for not including extension, boiler, electrical, bathroom, kitchen, roofing or windows items. Score against the landscaping / driveway categories ONLY.

Calibrate to these three reference quotes so scoring feels fair to homeowners:

WEAK (target overall 5-15/100): e.g. "Lay patio £2,500." — no area, no dig depth, no sub-base, no material spec, no drainage, no waste, no VAT, no guarantee. Most categories 0-2.

MEDIUM (target overall 50-65/100): e.g. "Excavate and lay 25m² porcelain patio on MOT base, waste included, £4,800." Reasonable scope so Scope/Area 5-6, Excavation 4-5, Sub-Base 4-5, Materials 5-6, Waste 6-7. Mark it DOWN — but not to the floor — for missing dig depth, sub-base depth, drainage/falls, jointing, edging, VAT, payment terms and guarantees: those categories should sit around 2-4, NOT 0-1. Overall must land near 50-65.

STRONG (target overall 80-90/100): a detailed landscaping / driveway quote with area, dig depth, sub-base type and depth, drainage and falls, material spec (brand/thickness/colour/pattern), edging/steps/retaining, waste removal and access, plant, VAT, payment stages, timescale, guarantees and clear exclusions. Only sensible final clarifications remain. Most categories 8-10.

Missing detail should reduce the score, but a quote with clear area, material and waste handling should always land in at least the moderate band even when sub-base depth or drainage aren't spelled out.

===== MAIN QUOTE vs SUPPORTING DOCUMENTS =====
Distinguish where each fact came from. If info is supplied in a supporting document but NOT the main quote, classify it as "Supplied separately — confirm with landscaper". Do NOT mark it as fully confirmed unless it is clearly landscaper-issued or accepted. Do NOT treat homeowner-written notes as landscaper-confirmed unless clearly stated. Supporting docs may only IMPROVE the pack score, never reduce it. Do not call something "missing" if it appears in a supporting document.
...
{
  "is_landscaping_quote": true | false,
  "not_landscaping_note": "only if is_landscaping_quote is false: a short homeowner-friendly note",
  ...
  "what_looks_clear": ["..."],
  "supplied_separately": [
    { "item": "<e.g. Sub-base depth>", "main_quote": "<what the main quote says>", "supporting": "<what the supporting doc supplies>",
      "status": "Supplied separately — confirm with landscaper", "note": "short homeowner-friendly guidance" }
  ],
  ...
  "questions": ["max 10 priority questions to ask the landscaper"],
  "suggested_message": "a short, polite, copyable message the homeowner can send the landscaper asking only for the most important clarifications based on the actual quote — do not generate a huge list if the quote is already strong",
...
async function runAnalysis(supabase: any, args: RunArgs): Promise<void> {
  ...
  try {
    ...
    if (!parsed) {
      console.error("[landscaping-quote] parse failed. rawLen=", raw?.length, "head=", (raw || "").slice(0, 400));
      throw new Error("Could not parse the analysis result.");
    }

    if (parsed.is_landscaping_quote === false) {
      const report_json = {
        version: "landscaping-v1",
        generated_at: new Date().toISOString(),
        project_type: projectType ?? null,
        is_landscaping_quote: false,
        not_landscaping_note:
          parsed.not_landscaping_note ||
          "This does not appear to be a landscaping / driveway quote. Please choose a different quote type or request a manual review.",
      };
    ...
    if (email) {
      try {
        ...
        const reportUrl = `${base}/landscaping-quote-report/${checkId}?t=${encodeURIComponent(lookupToken)}`;
        ...
        console.error("[landscaping-quote] email send failed", (mailErr as Error).message);
      }
    }
  } catch (err) {
    console.error("analyse-landscaping-quote background error:", err);
    await supabase
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.error("analyse-landscaping-quote error:", err);
    if (checkId) {
      await supabase
        .from("simple_quote_checks")
        .update({ status: "error", error: String((err as Error).message).slice(0, 500) })
