...
export interface KitchenSuppliedSeparatelyItem {
  item?: string;
  main_quote?: string;
  supporting?: string;
  status?: string;
...
export interface KitchenReportJson {
  version?: string;
  generated_at?: string;
  project_type?: string | null;
  is_kitchen_quote?: boolean;
  not_kitchen_note?: string;
  ...
  supplied_separately?: KitchenSuppliedSeparatelyItem[];
  not_found?: string[];
  key_risks?: string[];
  questions?: string[];
...
const SOURCE_LABEL: Record<string, string> = {
  ...
  addendum_clarification: "Supplied separately — confirm with kitchen installer",
  supplied_separately: "Supplied separately — confirm with kitchen installer",
...
export default function KitchenQuoteReport({ report }: { report: KitchenReportJson }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {suppliedSeparately.length ? (
        <Section title="Supplied Separately — Confirm With Kitchen Installer" icon={<ShieldCheck className="h-5 w-5 text-teal" />}>
          <p className="font-mono text-xs text-muted-foreground mb-3">
            These items are not in the main quote but appear in your supporting documents. Confirm the kitchen
      ...
      {/* Main Questions To Ask The Kitchen Installer */}
      {report.questions?.length ? (
        <Section title="Main Questions To Ask The Kitchen Installer" icon={<MessageSquare className="h-5 w-5 text-navy" />}>
      ...
      {/* Suggested Message To The Kitchen Installer */}
      {report.suggested_message ? (
        <div className="bg-navy rounded-2xl p-5 md:p-6 shadow-sm">
          <div className="flex items-center justify-between gap-2 mb-3">
            <h2 className="font-heading text-base md:text-lg text-white">Suggested Message To The Kitchen Installer</h2>
            <button
              onClick={copyMessage}
      ...
      <p className="text-center font-mono text-[11px] text-muted-foreground leading-relaxed px-4">
        ...
        qualified kitchen installer's assessment.
      </p>
    </div>
  );
}
