...
export interface WindowsDoorsCategory {
  key: string;
  name: string;
  relevant: boolean;
  score: number | null;
...
export interface WindowsDoorsSuppliedSeparatelyItem {
  item?: string;
  main_quote?: string;
  supporting?: string;
  status?: string;
...
export interface WindowsDoorsReportJson {
  version?: string;
  generated_at?: string;
  project_type?: string | null;
  is_windows_doors_quote?: boolean;
  not_windows_doors_note?: string;
  ...
  categories?: WindowsDoorsCategory[];
  quick_verdict?: string;
  what_looks_clear?: string[];
  supplied_separately?: WindowsDoorsSuppliedSeparatelyItem[];
  not_found?: string[];
  key_risks?: string[];
  questions?: string[];
...
const SOURCE_LABEL: Record<string, string> = {
  ...
  addendum_clarification: "Supplied separately — confirm with window & door installer",
  supplied_separately: "Supplied separately — confirm with window & door installer",
...
export default function WindowsDoorsQuoteReport({ report }: { report: WindowsDoorsReportJson }) {
  ...
  const band = getScoreBand(score, "installer");
  const theme = band;
  ...
  return (
    <div className="space-y-5">
      ...
      {suppliedSeparately.length ? (
        <Section title="Supplied Separately — Confirm With Window & Door Installer" icon={<ShieldCheck className="h-5 w-5 text-teal" />}>
          <p className="font-mono text-xs text-muted-foreground mb-3">
            These items are not in the main quote but appear in your supporting documents. Confirm the window &
            door installer agrees they form part of the agreed quote.
          </p>
        ...
        >
          <p className="font-mono text-xs text-muted-foreground mb-2">
            {score >= 80
              ...
              : "These items were not visible in the main quote or supporting documents. They may simply be outside the agreed scope — confirm with your window & door installer if required."}
          </p>
          <ul className="space-y-2">
      ...
      {report.questions?.length ? (
        <Section title="Main Questions To Ask The Window & Door Installer" icon={<MessageSquare className="h-5 w-5 text-navy" />}>
      ...
      <p className="text-center font-mono text-[11px] text-muted-foreground leading-relaxed px-4">
        ...
        qualified window & door installer's assessment.
      </p>
    </div>
  );
}
