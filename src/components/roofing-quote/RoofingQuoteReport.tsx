...
export interface RoofingCategory {
  key: string;
  name: string;
  relevant: boolean;
  score: number | null;
...
export interface RoofingSuppliedSeparatelyItem {
  item?: string;
  main_quote?: string;
  supporting?: string;
  status?: string;
...
export interface RoofingReportJson {
  version?: string;
  generated_at?: string;
  project_type?: string | null;
  is_roofing_quote?: boolean;
  not_roofing_note?: string;
  ...
  categories?: RoofingCategory[];
  quick_verdict?: string;
  what_looks_clear?: string[];
  supplied_separately?: RoofingSuppliedSeparatelyItem[];
  not_found?: string[];
  key_risks?: string[];
  questions?: string[];
...
const SOURCE_LABEL: Record<string, string> = {
  ...
  addendum_clarification: "Supplied separately — confirm with roofer",
  supplied_separately: "Supplied separately — confirm with roofer",
...
export default function RoofingQuoteReport({ report }: { report: RoofingReportJson }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {suppliedSeparately.length ? (
        <Section title="Supplied Separately — Confirm With Roofer" icon={<ShieldCheck className="h-5 w-5 text-teal" />}>
          <p className="font-mono text-xs text-muted-foreground mb-3">
            These items are not in the main quote but appear in your supporting documents. Confirm the roofer
      ...
      {report.not_found?.length ? (
        <Section title="Not Found / Confirm If Required" icon={<HelpCircle className="h-5 w-5 text-amber-600" />}>
          <p className="font-mono text-xs text-muted-foreground mb-2">
            ...
            agreed scope — confirm with your roofer if required.
          </p>
          <ul className="space-y-2">
      ...
      {/* Main Questions To Ask The Roofer */}
      {report.questions?.length ? (
        <Section title="Main Questions To Ask The Roofer" icon={<MessageSquare className="h-5 w-5 text-navy" />}>
      ...
      {/* Suggested Message To The Roofer */}
      {report.suggested_message ? (
        <div className="bg-navy rounded-2xl p-5 md:p-6 shadow-sm">
          <div className="flex items-center justify-between gap-2 mb-3">
            <h2 className="font-heading text-base md:text-lg text-white">Suggested Message To The Roofer</h2>
            <button
              onClick={copyMessage}
