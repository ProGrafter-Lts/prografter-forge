...
// ---- Types (self-contained — independent of other module reports) -----------
...
export interface BathroomSuppliedSeparatelyItem {
  item?: string;
  main_quote?: string;
  supporting?: string;
  status?: string;
...
export interface BathroomReportJson {
  version?: string;
  generated_at?: string;
  project_type?: string | null;
  is_bathroom_quote?: boolean;
  not_bathroom_note?: string;
  ...
  categories?: BathroomCategory[];
  quick_verdict?: string;
  what_looks_clear?: string[];
  supplied_separately?: BathroomSuppliedSeparatelyItem[];
  not_found?: string[];
  key_risks?: string[];
  questions?: string[];
...
const SOURCE_LABEL: Record<string, string> = {
  ...
  addendum_clarification: "Supplied separately — confirm with bathroom installer",
  supplied_separately: "Supplied separately — confirm with bathroom installer",
...
export default function BathroomQuoteReport({ report }: { report: BathroomReportJson }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* 3 / 4. Supplied Separately — Confirm With Bathroom Installer */}
      {suppliedSeparately.length ? (
        <Section title="Supplied Separately — Confirm With Bathroom Installer" icon={<ShieldCheck className="h-5 w-5 text-teal" />}>
          <p className="font-mono text-xs text-muted-foreground mb-3">
            These items are not in the main quote but appear in your supporting documents. Confirm the bathroom
            installer agrees they form part of the agreed quote.
          </p>
      ...
      {/* 4. Not Found / Confirm If Required (softened for strong quotes) */}
      {report.not_found?.length ? (
        <Section
          title={score >= 80 ? "Final Confirmations" : "Not Found / Confirm If Required"}
          icon={<HelpCircle className={`h-5 w-5 ${score >= 80 ? "text-teal" : "text-amber-600"}`} />}
        >
          <p className="font-mono text-xs text-muted-foreground mb-2">
            {score >= 80
              ? "Worth confirming before acceptance. These are minor points — not major issues, but useful to agree in writing."
              : "These items were not visible in the main quote or supporting documents. They may simply be outside the agreed scope — confirm with your bathroom installer if required."}
          </p>
          <ul className="space-y-2">
            {report.not_found.map((t, i) => (
              <li key={i} className="flex gap-2 font-mono text-sm text-navy/90">
                <HelpCircle className={`h-4 w-4 shrink-0 mt-0.5 ${score >= 80 ? "text-teal" : "text-amber-600"}`} />
                <span>{t}</span>
              </li>
            ))}
          </ul>
      ...
      {/* 6. Main Questions To Ask The Bathroom Installer */}
      {report.questions?.length ? (
        <Section title="Main Questions To Ask The Bathroom Installer" icon={<MessageSquare className="h-5 w-5 text-navy" />}>
      ...
      {/* 7. Suggested Message To The Bathroom Installer */}
      {report.suggested_message ? (
        <div className="bg-navy rounded-2xl p-5 md:p-6 shadow-sm">
          <div className="flex items-center justify-between gap-2 mb-3">
            <h2 className="font-heading text-base md:text-lg text-white">Suggested Message To The Bathroom Installer</h2>
            <button
              onClick={copyMessage}
      ...
      <p className="text-center font-mono text-[11px] text-muted-foreground leading-relaxed px-4">
        ...
        qualified bathroom installer's assessment.
      </p>
    </div>
  );
}
