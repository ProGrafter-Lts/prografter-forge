...
// ---- Types (self-contained — independent of the extension report) -----------
...
export interface BoilerSuppliedSeparatelyItem {
  item?: string;
  main_quote?: string;
  supporting?: string;
  status?: string;
...
export interface BoilerReportJson {
  version?: string;
  generated_at?: string;
  project_type?: string | null;
  is_boiler_quote?: boolean;
  not_boiler_note?: string;
  ...
  categories?: BoilerCategory[];
  quick_verdict?: string;
  what_looks_clear?: string[];
  supplied_separately?: BoilerSuppliedSeparatelyItem[];
  not_found?: string[];
  key_risks?: string[];
  questions?: string[];
...
const VERDICT_THEME: Record<string, { label: string; ring: string; text: string; bar: string }> = {
  low: { label: "Low confidence", ring: "ring-rose-200", text: "text-rose-700", bar: "bg-rose-500" },
  moderate: { label: "Moderate confidence", ring: "ring-amber-200", text: "text-amber-700", bar: "bg-amber-500" },
  good: { label: "Good confidence", ring: "ring-emerald-200", text: "text-emerald-700", bar: "bg-emerald-500" },
  strong: { label: "Strong confidence", ring: "ring-emerald-300", text: "text-emerald-700", bar: "bg-emerald-600" },
};

const SOURCE_LABEL: Record<string, string> = {
  in_quote: "In the main quote",
  supplied_in_supporting: "Supplied in supporting document",
  addendum_clarification: "Supplied separately — confirm with installer",
  supplied_separately: "Supplied separately — confirm with installer",
...
export default function BoilerQuoteReport({ report }: { report: BoilerReportJson }) {
  ...
  return (
    <div className="space-y-5">
      {/* 1. Boiler Quote Score + confidence */}
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-6">
          ...
          <div className="shrink-0 text-center">
            <p className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground mb-1">
              {hasDocs ? "Boiler Quote Score" : "Boiler Quote Score"}
            </p>
      ...
      {/* 3. Supplied Separately — Confirm With Installer */}
      {suppliedSeparately.length ? (
        <Section title="Supplied Separately — Confirm With Installer" icon={<ShieldCheck className="h-5 w-5 text-teal" />}>
          <p className="font-mono text-xs text-muted-foreground mb-3">
            These items are not in the main quote but appear in your supporting documents. Confirm the installer
      ...
      {/* 6. Questions To Ask The Installer */}
      {report.questions?.length ? (
        <Section title="Questions To Ask The Installer" icon={<MessageSquare className="h-5 w-5 text-navy" />}>
      ...
      {/* 7. Suggested Message To Installer */}
      {report.suggested_message ? (
        <div className="bg-navy rounded-2xl p-5 md:p-6 shadow-sm">
          <div className="flex items-center justify-between gap-2 mb-3">
            <h2 className="font-heading text-base md:text-lg text-white">Suggested Message To Your Installer</h2>
            <button
              onClick={copyMessage}
      ...
      {/* Optional detail — collapsed category scores */}
      ...
      <p className="text-center font-mono text-[11px] text-muted-foreground leading-relaxed px-4">
        ...
        Gas Safe registered engineer's assessment.
      </p>
    </div>
  );
}
