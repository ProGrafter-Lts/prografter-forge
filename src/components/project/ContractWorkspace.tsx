...
interface Props {
  jobId: string;
  /** Rendered inside a panel over the project screen — close returns to the project. */
  onClose?: () => void;
  /** Jump to the shared project-level Activity view, filtered to contract events. */
  onOpenActivity?: () => void;
}

/**
 * The contract itself — document, bespoke terms, variations and commission.
 * Photos and activity deliberately live at project level (single source of
 * truth); this workspace links across to them rather than duplicating them.
 */
const ContractWorkspace = ({ jobId, onClose, onOpenActivity }: Props) => {
  ...
  const load = async () => {
    ...
    const { data: h } = await supabase
      .from("homeowners")
      .select("user_id")
      .eq("id", c.homeowner_id)
      .maybeSingle();
    if (uid && h?.user_id === uid) setRole("homeowner");
    const { data: t } = await supabase.from("trades").select("user_id").eq("id", c.trade_id).maybeSingle();
    if (uid && t?.user_id === uid) setRole("trade");
    ...
    if (uid) {
      supabase
        .rpc("log_contract_event", { _contract_id: c.id, _event_type: "viewed", _payload: {} })
        .then(() => {});
    }
  ...
  if (!contract) {
    ...
    return (
      <div className="text-center max-w-md mx-auto bg-card border border-border rounded-2xl p-8 my-12">
        <ShieldAlert className="w-10 h-10 text-muted-foreground mx-auto mb-3" />
        <h2 className="font-heading text-2xl text-primary mb-2">{copy.title}</h2>
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="font-mono text-xs text-muted-foreground mt-1">
            ...
            {formatGBP(contract.total_value_incl_vat_pence)} incl. VAT
          </p>
        </div>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="font-mono text-xs text-amber-800 mt-1">
              Signing is temporarily disabled while our solicitor finalises the master template. You can
              review the document, add bespoke terms and discuss with the other party in the meantime.
            </p>
          </div>
        </div>
      )}
      ...
      {integrityOk === false && (
        <div className="bg-destructive/10 border border-destructive/30 rounded-xl p-4">
          <p className="font-mono text-xs text-destructive font-medium">
            This contract failed integrity verification. Please refresh the page. If this persists, contact
            support — do not sign.
          </p>
        </div>
      )}
      ...
      <Tabs defaultValue="document" className="w-full">
        <TabsList className="grid grid-cols-2 sm:grid-cols-4 w-full bg-card border border-border h-auto p-1">
          <TabsTrigger
            value="document"
            className="text-foreground data-[state=active]:bg-primary data-[state=active]:text-primary-foreground text-xs sm:text-sm"
          >
            Document
          </TabsTrigger>
          <TabsTrigger
        ...
        <TabsContent value="document" className="mt-4 space-y-4">
          <div className="bg-card border border-border rounded-2xl p-5">
            <h3 className="font-heading text-primary text-lg mb-3">Parties &amp; Property</h3>
          ...
          <div className="bg-card border border-border rounded-2xl p-5">
            <h3 className="font-heading text-primary text-lg mb-3">Payment milestones</h3>
              ...
              {milestones.map((m: any, i: number) => (
                ...
                >
                  ...
                  <span className="text-foreground font-medium shrink-0">
                    {formatGBP(Number(m.amount_pence) || 0)}
                  </span>
                </li>
              ))}
            </ol>
          ...
          <div className="bg-card border border-border rounded-2xl p-5">
            <h3 className="font-heading text-primary text-lg mb-3">Legal text</h3>
...
{contract.rendered_legal_text ?? "Legal text not yet rendered."}
          ...
          {/* Signing block */}
          <div className="bg-card border border-border rounded-2xl p-5">
            <h3 className="font-heading text-primary text-lg mb-3">Signatures</h3>
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <span className="block">
                        <Button onClick={signNow} disabled={!canSign || !allChecked || signing} className="w-full">
                          {signing ? (
                            <>
          ...
          {onOpenActivity && (
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg border border-border bg-card px-3 py-2 font-mono text-xs text-foreground hover:border-primary/50 transition-colors"
            >
              <Clock className="w-3.5 h-3.5" /> View contract audit trail in project activity
            </button>
          )}
        </TabsContent>
        ...
        <TabsContent value="bespoke" className="mt-4 space-y-4">
          <div className="bg-card border border-border rounded-2xl p-5">
            <h3 className="font-heading text-primary text-lg mb-1">Bespoke terms</h3>
            <p className="font-mono text-xs text-muted-foreground mb-4">
              Add up to 4,000 characters of additional terms. Saving will reset the other party's signature so
              they can re-review.
            </p>
...
{contract.trade_bespoke_terms || <span className="text-muted-foreground italic">None</span>}
            ...
            {role && ["draft", "pending_signatures", "awaiting_signatures"].includes(contract.status) && (
              <div className="space-y-3 border-t border-border pt-4">
                <p className="font-mono text-xs text-foreground font-medium">Edit your bespoke terms ({role})</p>
                <Textarea
                  value={bespokeDraft}
        ...
        <TabsContent value="variations" className="mt-4 space-y-4">
          <div className="bg-card border border-border rounded-2xl p-5">
            <h3 className="font-heading text-primary text-lg mb-3">Contract variations</h3>
            {variations.length === 0 ? (
        ...
        <TabsContent value="commission" className="mt-4 space-y-4">
          <div className="bg-card border border-border rounded-2xl p-5 space-y-4">
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <h3 className="font-heading text-primary text-lg">Platform commission</h3>
            ...
            <p className="font-mono text-xs text-muted-foreground">
              7.5% of the cumulative job value (original contract plus every accepted variation), capped at{" "}
              {formatGBP(COMMISSION_CAP_PENCE)} per job. Variations never push the total past the cap.
            </p>
