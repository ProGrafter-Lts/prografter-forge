...
export interface LandscapingSuppliedSeparatelyItem {
  item?: string;
  main_quote?: string;
  supporting?: string;
  status?: string;
...
export interface LandscapingReportJson {
  version?: string;
  generated_at?: string;
  project_type?: string | null;
  is_landscaping_quote?: boolean;
  not_landscaping_note?: string;
  ...
  categories?: LandscapingCategory[];
  quick_verdict?: string;
  what_looks_clear?: string[];
  supplied_separately?: LandscapingSuppliedSeparatelyItem[];
  not_found?: string[];
  key_risks?: string[];
  questions?: string[];
...
const SOURCE_LABEL: Record<string, string> = {
  ...
  addendum_clarification: "Supplied separately — confirm with landscaper",
  supplied_separately: "Supplied separately — confirm with landscaper",
...
export default function LandscapingQuoteReport({ report }: { report: LandscapingReportJson }) {
  ...
  const band = getScoreBand(score, "landscaper");
  const theme = band;
  ...
  return (
    <div className="space-y-5">
      ...
      {suppliedSeparately.length ? (
        <Section title="Supplied Separately — Confirm With Landscaper" icon={<ShieldCheck className="h-5 w-5 text-teal" />}>
          <p className="font-mono text-xs text-muted-foreground mb-3">
            These items are not in the main quote but appear in your supporting documents. Confirm the landscaper
      ...
      {report.not_found?.length ? (
        <Section title={score >= 80 ? "Final Confirmations" : "Not Found / Confirm If Required"} icon={<HelpCircle className={`h-5 w-5 ${score >= 80 ? "text-teal" : "text-amber-600"}`} />}>
          <p className="font-mono text-xs text-muted-foreground mb-2">
            ...
            agreed scope — confirm with your landscaper if required.
          </p>
          <ul className="space-y-2">
      ...
      {report.questions?.length ? (
        <Section title="Main Questions To Ask The Landscaper" icon={<MessageSquare className="h-5 w-5 text-navy" />}>
      ...
      <SuggestedMessageBlock report={report} title="Suggested Message To The Landscaper" />
      ...
      <p className="text-center font-mono text-[11px] text-muted-foreground leading-relaxed px-4">
        ...
        qualified landscaper's assessment.
      </p>
    </div>
  );
}
