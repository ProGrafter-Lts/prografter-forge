...
export interface PlasteringCategory {
  key: string;
  name: string;
  relevant: boolean;
  score: number | null;
...
export interface PlasteringSuppliedSeparatelyItem {
  item?: string;
  main_quote?: string;
  supporting?: string;
  status?: string;
...
export interface PlasteringReportJson {
  version?: string;
  generated_at?: string;
  project_type?: string | null;
  is_plastering_quote?: boolean;
  not_plastering_note?: string;
  ...
  categories?: PlasteringCategory[];
  quick_verdict?: string;
  what_looks_clear?: string[];
  supplied_separately?: PlasteringSuppliedSeparatelyItem[];
  not_found?: string[];
  key_risks?: string[];
  questions?: string[];
...
const SOURCE_LABEL: Record<string, string> = {
  ...
  addendum_clarification: "Supplied separately — confirm with plasterer",
  supplied_separately: "Supplied separately — confirm with plasterer",
...
export default function PlasteringQuoteReport({ report }: { report: PlasteringReportJson }) {
  ...
  const band = getScoreBand(score, "plasterer");
  const theme = band;
  ...
  return (
    <div className="space-y-5">
      ...
      {suppliedSeparately.length ? (
        <Section title="Supplied Separately — Confirm With Plasterer" icon={<ShieldCheck className="h-5 w-5 text-teal" />}>
          <p className="font-mono text-xs text-muted-foreground mb-3">
            These items are not in the main quote but appear in your supporting documents. Confirm the plasterer
        ...
        >
          <p className="font-mono text-xs text-muted-foreground mb-2">
            {score >= 78
              ...
              : "These items were not visible in the main quote or supporting documents. They may simply be outside the agreed scope — confirm with your plasterer if required."}
          </p>
          <ul className="space-y-2">
      ...
      {report.questions?.length ? (
        <Section title="Main Questions To Ask The Plasterer" icon={<MessageSquare className="h-5 w-5 text-navy" />}>
      ...
      <SuggestedMessageBlock report={report} title="Suggested Message To The Plasterer" />
      ...
      <p className="text-center font-mono text-[11px] text-muted-foreground leading-relaxed px-4">
        ...
        qualified plasterer's assessment.
      </p>
    </div>
  );
}
