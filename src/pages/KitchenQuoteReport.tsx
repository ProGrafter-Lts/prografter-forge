...
import KitchenQuoteReport, { type KitchenReportJson } from "@/components/kitchen-quote/KitchenQuoteReport";
import { Loader2, AlertTriangle } from "lucide-react";

const KitchenQuoteReportPage = () => {
  const { id } = useParams<{ id: string }>();
  const [searchParams] = useSearchParams();
  const token = searchParams.get("t") || "";

  const [status, setStatus] = useState<"loading" | "ready" | "error">("loading");
  const [report, setReport] = useState<KitchenReportJson | null>(null);
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (data?.status === "complete" && data?.report_json) {
          setReport(data.report_json as KitchenReportJson);
          setStatus("ready");
          return;
        }
  ...
  const notKitchen = status === "ready" && report && report.is_kitchen_quote === false;

  return (
    <AppShell>
      <SEO
        title="Your Kitchen Quote Check Report | ProGrafter"
        description="Your kitchen quote check report."
        path="/kitchen-quote-report"
        noindex
      />
      <div className="max-w-3xl mx-auto px-4 pt-24 pb-10 md:pt-28 md:pb-14">
        ...
        {status === "error" && (
          <div className="text-center py-20 space-y-3">
            ...
            <a href="/quote-checker?module=kitchen" className="font-mono text-sm text-teal underline">Start a new check</a>
          </div>
        )}
        {notKitchen && (
        ...
        {status === "ready" && report && !notKitchen && (
          <>
            <div className="mb-6">
              <h1 className="font-heading text-2xl md:text-3xl text-navy">Your Kitchen Quote Check</h1>
              <p className="font-mono text-xs text-muted-foreground mt-1">
                A simple, quote-focused review of your kitchen quote.
              </p>
            </div>
            <KitchenQuoteReport report={report} />
          </>
        )}
      </div>
    </AppShell>
...
export default KitchenQuoteReportPage;
