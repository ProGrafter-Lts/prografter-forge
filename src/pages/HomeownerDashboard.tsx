...
const HomeownerDashboard = () => {
  ...
  const loadData = async (userId: string) => {
    ...
    const [quoteRes, variationRes, updatesRes] = await Promise.all([
      supabase
        .from("quotes")
        .select("id, amount, message, status, created_at, trade_id, job_id, ai_verdict, ai_verdict_summary, tier_enabled, budget_price, budget_description, standard_price, standard_description, premium_price, premium_description, selected_tier, vat_status, valid_until, estimated_start_date, estimated_duration_text, scope_of_works, exclusions, assumptions, deposit_required, deposit_amount, payment_schedule, certifications, pdf_path, trades:trades_public!quotes_trade_id_fkey(name, company_name, verified, review_count, avg_rating, tier, trade_type, cps_scheme, cps_registration_number, gas_safe_number), jobs(title, job_type)")
        .in("job_id", jobIds)
        .order("created_at", { ascending: false }),
      supabase
        .from("variations")
    ...
    <div className="min-h-screen dashboard-dark flex">
      ...
      <main className="flex-1 p-4 md:p-8 overflow-auto">
        <div className="max-w-5xl mx-auto space-y-8">
          ...
          {activeNav === "grants" && (
            <section>
              <h2 className="font-heading text-green-500 text-2xl mb-4 flex items-center gap-2">
