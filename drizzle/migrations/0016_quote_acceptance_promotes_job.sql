-- 1. Internal contract generator (no caller identity check)
...
REVOKE ALL ON FUNCTION public.create_contract_for_quote_internal(uuid, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.create_contract_for_quote_internal(uuid, uuid) TO service_role;

-- 2. Public entry point keeps the homeowner-only guard, delegates the work
...
-- 3. Allow system-driven job_match status transitions inside our triggers
...
BEGIN
  IF public.has_role(auth.uid(), 'admin')
     ...
     OR COALESCE(current_setting('prografter.system_update', true), '') = 'on' THEN
    RETURN NEW;
  END IF;
...
-- 4. Full promotion on acceptance (online or offline)
...
DECLARE
  v_is_win boolean;
BEGIN
  v_is_win := NEW.status IN ('accepted', 'agreed_offline')
    AND (TG_OP = 'INSERT' OR COALESCE(OLD.status, '') NOT IN ('accepted', 'agreed_offline'));

  IF NOT v_is_win THEN RETURN NEW; END IF;

  PERFORM set_config('prografter.system_update', 'on', true);
  ...
  -- b. create the contract (never block the acceptance if it fails)
  BEGIN
    PERFORM public.create_contract_for_quote_internal(NEW.id, NULL);
  EXCEPTION WHEN OTHERS THEN
    INSERT INTO public.job_escalation_events (job_id, event_type, notes)
    VALUES (NEW.job_id, 'contract_generation_failed',
            'Quote ' || NEW.id::text || ': ' || SQLERRM);
  END;

  -- c. supersede the other quotes still waiting on this job
  ...
  UPDATE public.job_trade_invitations
     SET status = 'closed', responded_at = COALESCE(responded_at, NOW())
   WHERE job_id = NEW.job_id
     AND trade_id <> NEW.trade_id
     AND status NOT IN ('quote_submitted', 'declined', 'closed');

  -- d. settle job matches
  UPDATE public.job_matches
     SET status = CASE WHEN trade_id = NEW.trade_id THEN 'won' ELSE 'closed' END
   WHERE job_id = NEW.job_id
     AND status NOT IN ('won', 'closed');

  PERFORM set_config('prografter.system_update', 'off', true);
...
FOR EACH ROW EXECUTE FUNCTION public.on_quote_accepted_advance_job();